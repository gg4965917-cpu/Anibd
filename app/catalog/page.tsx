...
import { filterAnime, getGenres, resolveGenreIds } from "@/lib/anime";
...
export default async function CatalogPage({
  ...
  const { items, hasNextPage, page } = await filterAnime({
    ...
    orderBy: sp.order_by,
    sort:
      sp.order_by === "score" || sp.order_by === "popularity" || !sp.order_by
        ? "desc"
        : "asc",
    ...
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <header className="mb-6">
        ...
        <p className="text-sm text-slate-400">
          Знайдіть аніме за жанром, роком чи типом. Метадані: MyAnimeList (Jikan) + AniList.
        </p>
      </header>
